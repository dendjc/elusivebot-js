...
exports.run = async (client, message, args) => {
  if(!message.member.permissions.has("ADMINISTRATOR")) return;  if(!message.member.permissions.has("ADMINISTRATOR")) return;

  let member = message.guild.members.cache.get(client.user.id);
  if(member.voice.channel) member.voice.channel.leave();
  if(!message.member.voice.channel && member.voice.channel) return member.voice.channel.leave();
  if(!message.member.voice.channel) return;

  const play = async (conn, i=0) => {
    //if(!songs[i]) i = 0
    // const dispatcher = await conn.play(await ytdl(songs[i]), { type: 'opus' })
    const dispatcher = await conn.play("http://live6.okradio.net:8054/", { filter: "audioonly" });
