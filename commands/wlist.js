...
exports.run = async (client, message, args) => {
  ...
  let warns = await db.all().filter(data => data.ID.startsWith(`warns_${message.guild.id}`)).sort((a,b) => b.data - a.data);
  let content = "";
  
  for(let i = 0; i < warns.length; i++) {
    let user = client.users.cache.get(warns[i].ID.split("_")[2]) || await client.users.fetch(warns[i].ID.split("_")[2]);
