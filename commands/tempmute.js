...
exports.run = async (client, message, args) => {
  ...
  if (message.member === member)
    return message.channel.send("Ne možeš sebe mutirati!");
  if (member.permissions.has("MANAGE_MESSAGES") && !allowed2)
    return message.channel.send("Ne možeš mutirati člana STAFFa!");

  let vrijeme = args[1];
  if (!vrijeme)
    return message.channel.send(
      "Nisi napisao/la vrijeme u satima (h), minutama (min) ili sekundama (sec)!"
    );
  let vrijemeint = parseInt(args[1]);
  if (
    (!vrijeme.includes("h") &&
      !vrijeme.includes("min") &&
      !vrijeme.includes("sec")) ||
    vrijemeint < 0 ||
    vrijemeint > 60
  )
    return message.channel.send("Nisi pravilno napisao/la vrijeme.");
  ...
  if (
    ...
    return message.channel.send(
      "Ne možeš koristiti kombinaciju vremenskih oznaka!"
    );
  ...
  if (member.roles.cache.has(muted.id))
    return message.channel.send("Taj član je već mutiran!");
        ...
        .catch(err => {
          ...
          if (i == 0) {
            let errorEmbed = new Discord.MessageEmbed()
              .setTitle(
                "Istekao je privremeni mute člana " +
                  user.tag +
                  ", ali se pojavio error!"
              )
              .addField("Error", err)
              .setThumbnail(user.displayAvatarURL())
              .setFooter(
  ...
  member.roles
    ...
    .catch(err =>
      message.channel.send("Nisam mogao mutirati tog člana zbog: " + err)
    );
};
exports.conf = {
