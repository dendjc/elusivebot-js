...
exports.run = async (client, message, args) => {
  ...
  let members = await message.guild.roles.cache.find(r => r.name === "Muted").members.map(m => m.user);
  let content = "";
  
  for(let i = 0; i < members.length; i++) {
    let mutetime = await db.fetch(`mutetime_${message.guild.id}_${members[i].id}`);
    if(mutetime === null) content += `${i+1}. ${members[i].tag}\n`;
    if(mutetime !== null) {
      let time = ms(mutetime.time - (Date.now() - mutetime.date));
      content += `${i+1}. ${members[i].tag} - vrijeme: ${time.days}d ${time.hours}h ${time.minutes}min ${time.seconds}s\n`;
    }
  }
